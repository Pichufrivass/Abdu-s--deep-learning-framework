...
capa_convulsion::capa_convulsion(int fk, int ck, int nf, int canales_entrada) {
    for (int i = 0; i < nf; i++) {
        filtros.push_back(make_shared<kernel>(fk, ck, canales_entrada));
    }
}

vector<vector<vector<shared_ptr<value>>>> capa_convulsion::operator()(vector<vector<vector<shared_ptr<value>>>>& imagenes, int fi, int ci, int fk, int ck, int mfk, int mck) {
...
vector<shared_ptr<value>> capa_convulsion::parameter() {
    vector<shared_ptr<value>> p;
    for (size_t i = 0; i < filtros.size(); i++) {
        vector<shared_ptr<value>> fp = filtros[i]->parameter();
        p.insert(p.end(), fp.begin(), fp.end());
    }
    return p;
}

CNN::CNN(int fi, int ci, int fk, int ck, int nf, int nc, int mfk, int mck) {
    int alto = ci;
    int ancho = fi;
    for (int i = 0; i < nc; i++) {
        ancho = ancho - fk + 1;
...
vector<vector<vector<shared_ptr<value>>>> CNN::operator()(vector<vector<vector<shared_ptr<value>>>>& imagen, int fi, int ci, int fk, int ck, int mfk, int mck) {
...
vector<shared_ptr<value>> CNN::parameter() {
    vector<shared_ptr<value>> p;
    for (size_t i = 0; i < capas.size(); i++) {
        vector<shared_ptr<value>> cp = capas[i]->parameter();
